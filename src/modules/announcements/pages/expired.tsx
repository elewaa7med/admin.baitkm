...
@byPrivateRoute(ROUTES.ANNOUNCEMENTS_EXPIRED)
class Rejected extends React.Component<{}, {}> {
    public state: IState = {
        ...
        form: {
            ...
            landType: null,
            commercialType: null,
            announcementStatus: AnnouncementStatusEnum.Expired,
            features: [],
            priceFrom: 0,
            priceTo: 0,
            minArea: 0,
    ...
    public changeFilters = async (form: IAddSaveFilterRequestModel) => {
        ...
        window.routerHistory.replace(query ? `${ROUTES.ANNOUNCEMENTS_EXPIRED}?${query}` : ROUTES.ANNOUNCEMENTS_EXPIRED);
        this.setState({ form });
    ...
    private columnConfig = [
        {
            ...
            style: { minWidth: 370 },
            cell: (row: any) =>
                <Link to={ROUTES.ANNOUNCEMENT_DETAILS_EXPIRED.replace(':id', row.id.toString())}>    <div className='B-announcement-user'>
    ...
    // private  getAnnouncementById  = async (announcementId: number)=>{
    //     if(announcementId === 0){
    //         this.changeFilters(this.state.form)
    //     }else{
    //         const result = await AnnouncementController.FilterById(announcementId);
    //         if(result.success){
    //             this.setState(async()=>{
    //                 if(result.data.data){
    //                     const arr = Object.values(result.data.data);
                    ...
                    <div className='B-announcement-list'>
                        {(data.data && data.data.length !== 0) ?
                            ...
                            : <div className='B-data-empty'>
                                ...
                                <p>Expired announcement list is empty</p>
                            </div>}
