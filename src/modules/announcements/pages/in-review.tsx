...
@byPrivateRoute(ROUTES.ANNOUNCEMENTS_IN_REVIEW)
class InReview extends React.Component<{}, {}> {
    public state: IState = {
        ...
        form: {
            ...
            landType: null,
            commercialType: null,
            announcementStatus: AnnouncementStatusEnum.InReview,
            features: [],
            priceFrom: 0,
            priceTo: 0,
            minArea: 0,
    ...
    public changeFilters = async (form: IAddSaveFilterRequestModel) => {
        ...
        window.routerHistory.replace(query ? `${ROUTES.ANNOUNCEMENTS_IN_REVIEW}?${query}` : ROUTES.ANNOUNCEMENTS_IN_REVIEW);
        this.setState({ form });
        this.reqDate = new Date();
        const date = this.reqDate;
        const result = await AnnouncementController.FilterAnnouncement({ page: 1, count: 10, announcementFilter: form });
        if (date.getTime() === this.reqDate.getTime()) {
            this.setState(async () => {
                if (result.data.data) {
                    const arr = Object.values(result.data.data);
                    arr.map((item: IUserApproved) => {
    ...
    private columnConfig = [
        {
            ...
            style: { minWidth: 370 },
            cell: (row: any) =>
                <Link to={ROUTES.ANNOUNCEMENTS_DETAILS_IN_REVIEW.replace(':id', row.id.toString())}>
                    ...
                    <div className='B-announcement-list'>
                        {(data.data && data.data.length !== 0) ?
                            ...
                            : <div className='B-data-empty'>
                                ...
                                <p>In review announcement list is empty</p>
                            </div>}
                    </div>
